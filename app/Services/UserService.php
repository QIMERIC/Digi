...
class UserService extends Service
{
    ...
    */
     ...
     * @return bool
     */
    public function updateHeader($header, $user)
    {
        DB::beginTransaction();

        try {
            if(!$header) throw new \Exception ("Please upload a file.");
            $filename = $user->id . '.' . $header->getClientOriginalExtension();

            if ($user->header !== 'default.jpg') {
                $file = 'images/headers/' . $user->header;
                //$destinationPath = 'uploads/' . $id . '/';

                if (File::exists($file)) {
                    if(!unlink($file)) throw new \Exception("Failed to unlink old header.");
                }
            }

            // Checks if uploaded file is a GIF
            if ($header->getClientOriginalExtension() == 'gif') {

                if(!copy($header, $file)) throw new \Exception("Failed to copy file.");
                if(!$file->move( public_path('images/headers', $filename))) throw new \Exception("Failed to move file.");
                if(!$header->move( public_path('images/headers', $filename))) throw new \Exception("Failed to move file.");

            }

            else {
                if(!Image::make($header)->resize(150, 150)->save( public_path('images/headers/' . $filename)))
                throw new \Exception("Failed to process header.");
            }

            $user->header = $filename;
            $user->save();

            return $this->commitReturn($header);
        } catch(\Exception $e) {
            $this->setError('error', $e->getMessage());
        }
        return $this->rollbackReturn(false);
